...
export default function ProjectDrawer({
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            {/* Elevator Pitch */}
            <p style={{ color: '#4b5563', lineHeight: '1.7', fontSize: '0.95rem', margin: '0 0 1.25rem 0' }}>{selectedProject.elevator_pitch}</p>

            {/* Meta Info */}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#f3f4f6', borderRadius: '8px', fontSize: '0.875rem', color: '#374151' }}>
                <span className="material-symbols-outlined" style={{ fontSize: '1.1rem', color: '#4F46E5' }}>group</span>
                <span><strong>{selectedProject.teammates_needed}</strong> teammates needed</span>
              </div>
              {selectedProject.is_approved && selectedProject.admin_profile?.full_name && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#d1fae5', borderRadius: '8px', fontSize: '0.875rem', color: '#065f46' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '1.1rem' }}>verified</span>
                  <span>Approved by <strong>{selectedProject.admin_profile.full_name}</strong></span>
                </div>
              )}
            </div>
            ...
            {selectedProject.skills_needed && selectedProject.skills_needed.length > 0 && (
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ margin: '0 0 0.75rem 0', fontSize: '0.875rem', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Skills Needed</h4>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {selectedProject.skills_needed.map((tag, idx) => (
                    <span key={idx} style={{ padding: '0.35rem 0.75rem', background: '#e0e7ff', color: '#3730a3', borderRadius: '6px', fontSize: '0.8rem', fontWeight: '500' }}>{tag}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {(activeTab === 'marketplace' || activeTab === 'manage-ideas') && (
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div
                  ...
                  onClick={() => navigate(`/profile/${selectedProject.author_id}`)}
                >
                  <div style={{
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    {currentUserProfile?.user_role === 'admin' && activeTab === 'manage-ideas' && (
                      <button
                        onClick={(e) => { e.stopPropagation(); alert('Messaging feature coming soon!'); }}
                        style={{ background: '#e0e7ff', border: 'none', cursor: 'pointer', color: '#4F46E5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.5rem', borderRadius: '8px' }}
                        title="Message User"
                      >
            ...
            {activeTab === 'marketplace' ? (
              <>
                <button onClick={() => setIsApplying(true)} style={{ flex: 1, padding: '0.75rem', background: '#4F46E5', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '1.1rem' }}>group_add</span>
                ...
                <button style={{ padding: '0.75rem', background: '#f3f4f6', color: '#374151', border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Report">
                  <span className="material-symbols-outlined" style={{ fontSize: '1.1rem' }}>flag</span>
                </button>
              </>
            ) : (
              <button style={{ flex: 1, padding: '0.75rem', background: '#f3f4f6', color: '#374151', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.9rem' }} onClick={() => navigate('/post-idea')}>
                Edit Idea (Coming Soon)
              </button>
            )}
          </div>
        </div>
      ...
      {/* Application Modal (sits on top of details modal) */}
      {isApplying && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', color: '#1f2937', padding: '2rem', borderRadius: '16px', width: '90%', maxWidth: '500px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            <h3 style={{ margin: '0 0 1.5rem 0', color: '#111827', fontSize: '1.5rem', fontWeight: 'bold' }}>Apply to Join "{selectedProject?.project_title}"</h3>

            <form onSubmit={submitApplication}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <input type="text" value={currentUserProfile?.full_name || ''} disabled style={{ flex: 1, padding: '0.75rem', background: '#f3f4f6', border: '1px solid #d1d5db', color: '#6b7280', borderRadius: '8px' }} placeholder="Name" />
                  <input type="tel" value={currentUserProfile?.mobile_number || ''} disabled style={{ flex: 1, padding: '0.75rem', background: '#f3f4f6', border: '1px solid #d1d5db', color: '#6b7280', borderRadius: '8px' }} placeholder="Contact Number" />
                </div>
                <input type="email" value={currentUserProfile?.email || ''} disabled style={{ padding: '0.75rem', background: '#f3f4f6', border: '1px solid #d1d5db', color: '#6b7280', borderRadius: '8px' }} placeholder="Email" />

                <input type="url" required placeholder="LinkedIn Profile URL" value={applyForm.linkedin} onChange={e => setApplyForm({ ...applyForm, linkedin: e.target.value })} style={{ padding: '0.75rem', background: '#fff', border: '1px solid #d1d5db', color: '#111827', borderRadius: '8px', outline: 'none' }} />
                <textarea required placeholder="Your Bio (Skills & Experience)" rows="3" value={applyForm.bio} onChange={e => setApplyForm({ ...applyForm, bio: e.target.value })} style={{ padding: '0.75rem', background: '#fff', border: '1px solid #d1d5db', color: '#111827', borderRadius: '8px', resize: 'none', outline: 'none' }}></textarea>
                <textarea required placeholder="Why do you want to join this project?" rows="3" value={applyForm.reason} onChange={e => setApplyForm({ ...applyForm, reason: e.target.value })} style={{ padding: '0.75rem', background: '#fff', border: '1px solid #d1d5db', color: '#111827', borderRadius: '8px', resize: 'none', outline: 'none' }}></textarea>
              </div>
