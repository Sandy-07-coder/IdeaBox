-- SQL for Role-Based Idea Approval System
...
-- 2. Add approval-related columns to ideas table (if it exists)
-- If your ideas table has a different name, update accordingly
...
CREATE INDEX IF NOT EXISTS idx_ideas_user_id_approved ON public.ideas(user_id, is_approved);
...
-- 5. Update existing ideas to be approved if they have an owner
-- This assumes ideas already created should be visible
UPDATE public.ideas
...
-- SELECT id, title, user_id, is_approved, created_at FROM public.ideas WHERE is_approved = false ORDER BY created_at DESC;
